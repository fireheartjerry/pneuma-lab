...
def test_tier_constants_fail_closed(field: str, value: str) -> None:
    record = architecture(); record[field] = value
    with pytest.raises(CloudManifestError): validate_architecture(record)


def test_controller_cannot_renew_lease_or_share_watcher_identity() -> None:
    record = architecture(); record["controller_policy"]["actions"].append("dynamodb:UpdateItem")
    with pytest.raises(CloudManifestError): validate_architecture(record)
    record = architecture(); record["watcher_policy"]["identity"] = "controller"
    with pytest.raises(CloudManifestError): validate_architecture(record)
