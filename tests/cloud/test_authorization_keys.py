...
def test_committed_registry_trusts_only_the_project_scoped_cloudshell_key() -> None:
    ...
    registry = validate_key_registry(json.loads((FIXTURES / "approver-key-registry.json").read_text()))
    ...
    with pytest.raises(CloudManifestError, match="not in the trusted registry"):
        resolve_trusted_key(registry, "approver-primary", at=NOW)


def test_committed_candidate_carries_no_authority() -> None:
    record = validate_retrieval_authorization(json.loads((FIXTURES / "retrieval-authorization-candidate.json").read_text()))
...
def test_a_candidate_can_never_retrieve() -> None:
    lock = _lock()
    record = json.loads((FIXTURES / "retrieval-authorization-candidate.json").read_text())
    ...
    with pytest.raises(CloudManifestError, match="not a candidate"):
        require_authorized(record, lock, key_registry=registry, at=NOW, ledger_path=LEDGER)
...
def test_a_properly_signed_authorization_verifies_and_retrieves() -> None:
    private = _keypair()
    lock = _lock()
    record = _authorization(private, lock)
    registry = _registry(private)

    assert verify_signature(record, registry, at=NOW)["key_id"] == "approver-primary"
    assert verify_ledger_binding(record, LEDGER) == _ledger_row_digest()
    assert require_authorized(record, lock, key_registry=registry, at=NOW, ledger_path=LEDGER)["status"] == "authorized"

    # A fetcher that returns exactly the pinned bytes for each planned step.
    def fetcher(step: dict) -> bytes:
        return _payload_for(step["expected_sha256"])

    receipts = retrieve_and_verify(lock, record, fetcher, key_registry=registry, at=NOW, ledger_path=LEDGER)
    assert receipts and all(item["sha256"] for item in receipts)


_PAYLOADS: dict[str, bytes] = {}


def _payload_for(expected: str) -> bytes:
    """Return bytes hashing to `expected`, found by search over a tiny space.

    The plan's digests come from `_real_parts`, which derives them from short
    seeds, so this simply re-derives the same preimages instead of inventing a
    hash collision.
    """

    if not _PAYLOADS:
        for seed in ("model", "tokenizer", "benchmark", "verifier", "contamination", "licence", "task-manifest", "image"):
            _PAYLOADS[hashlib.sha256(seed.encode()).hexdigest()] = seed.encode()
    return _PAYLOADS[expected]


# ---------------------------------------------------------------------------
# Byte-ceiling enforcement and artifact verification.
#
# Restored during hostile review: these controls lost their coverage when the
# retrieval tests were rewritten for the authenticated signature, and a ceiling
# with no test is a ceiling nobody will notice breaking.
# ---------------------------------------------------------------------------


def _uniform_lock(payload: bytes) -> dict:
    """A real candidate lock whose every pinned digest is `sha256(payload)`.

    Receipt *paths* stay distinct, since a real candidate lock may not point two
    artifacts at one destination; only the expected content digests coincide, so
    a single payload satisfies every step of the plan.
    """
...
def test_authorized_retrieval_verifies_every_artifact_and_fails_on_a_swap() -> None:
    ...
    record = _authorization(private, lock)
    registry = _registry(private)
    context = {"key_registry": registry, "at": NOW, "ledger_path": LEDGER}

    receipts = retrieve_and_verify(lock, record, lambda step: payload, **context)
    ...
    # A substituted byte fails the pinned digest, so nothing is accepted.
    with pytest.raises(CloudManifestError, match="do not match the pinned digest"):
        retrieve_and_verify(lock, record, lambda step: b"substituted-bytes", **context)


def test_declared_size_over_ceiling_is_refused_before_any_fetch() -> None:
    """An over-budget step is refused rather than transferred and then reported."""
    ...
    with pytest.raises(CloudManifestError, match="declared retrieval size exceeds"):
        retrieve_and_verify(
            lock, record, fetcher,
            key_registry=_registry(private), at=NOW, ledger_path=LEDGER,
            declared_sizes={hashlib.sha256(payload).hexdigest(): 1_000_000},
        )
    assert calls == []


def test_post_fetch_ceiling_backstops_an_undeclared_overrun() -> None:
    """A fetcher returning more than it declared is stopped at the first step."""

    payload = b"pinned-bytes"
    private = _keypair()
    lock = _uniform_lock(payload)
    record = _authorization(private, lock, byte_ceiling_bytes=1)
    calls: list[str] = []
    ...
    with pytest.raises(CloudManifestError, match="exceeded the authorized byte ceiling"):
        retrieve_and_verify(lock, record, fetcher, key_registry=_registry(private), at=NOW, ledger_path=LEDGER)
    assert calls == ["model"]
...
def test_an_unenumerated_key_authorizes_nothing_even_with_a_valid_signature() -> None:
    ...
    with pytest.raises(CloudManifestError, match="not in the trusted registry"):
        verify_signature(record, {**stranger, "keys": [{**stranger["keys"][0], "key_id": "other-key"}]}, at=NOW)
...
def test_a_revoked_key_is_refused() -> None:
    ...
    with pytest.raises(CloudManifestError, match="was revoked"):
        verify_signature(record, registry, at=NOW)
...
def test_a_key_outside_its_validity_window_is_refused(at: str) -> None:
    ...
    with pytest.raises(CloudManifestError, match="outside its validity window"):
        verify_signature(record, _registry(private), at=at)
...
def test_an_expired_authorization_is_refused() -> None:
    ...
    with pytest.raises(CloudManifestError, match="expired at"):
        verify_signature(record, _registry(private), at="2026-08-08T00:00:00Z")
...
def test_an_authorization_is_not_valid_before_it_was_granted() -> None:
    ...
    with pytest.raises(CloudManifestError, match="not yet valid"):
        verify_signature(record, _registry(private), at="2026-07-30T00:00:00Z")
...
def test_fractional_seconds_are_ordered_as_instants_not_as_strings() -> None:
    ...
    with pytest.raises(CloudManifestError, match="expired at"):
        verify_signature(record, _registry(private), at="2026-08-06T00:00:00.001Z")
    ...
    assert verify_signature(record, registry, at="2026-08-01T00:00:00Z")["key_id"] == "approver-primary"
...
def test_a_malformed_timestamp_fails_closed() -> None:
    ...
    with pytest.raises(CloudManifestError, match="malformed UTC timestamp"):
        verify_signature(record, _registry(private), at="not-a-timestamp")
...
def test_an_authorization_that_expires_before_it_is_granted_is_refused() -> None:
    ...
    with pytest.raises(CloudManifestError, match="expires no later than it was granted"):
        verify_signature(record, _registry(private), at=NOW)
...
def test_a_signature_over_a_different_record_does_not_transfer() -> None:
    ...
    with pytest.raises(CloudManifestError):
        verify_signature(second, _registry(private), at=NOW)
...
def test_a_valid_signature_over_a_synthetic_lock_still_refuses() -> None:
    ...
    private = _keypair()
    synthetic = json.loads((FIXTURES / "input-lock-fixture.json").read_text())
    ...
    with pytest.raises(CloudManifestError, match="synthetic demonstration"):
        require_authorized(record, synthetic, key_registry=registry, at=NOW, ledger_path=LEDGER)
