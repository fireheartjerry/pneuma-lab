...
| Decisions and supersessions | 15, latest applicable DL row | DL-44 resolves the remaining sample ranges without claiming power |
...
| --- | --- | --- | --- | --- | --- |
| H1 notice superiority to all five comparators | $\Delta_a^N=\nu(p)-\nu(a)$ for five $a$ | A1–A5, A7–A9; Theorem 1 identifies finite roster; notice construct validity remains A5 | Fixed cell→lineage→motif difference; whole-lineage bootstrap and max-T valid under Lemma 1 | Five component p-values inside $p_{H1}$, validity gates, lower bounds, Holm slot | T1 |
| H1 repeat-harm reduction to all five comparators | $\Delta_a^B=\mu(a)-\mu(p)$ for five $a$ | A1–A4, A6, A8–A9; Theorem 1; superpopulation step is A6 | Paired complete-block hierarchy; Fisher sharp-null p-value by Proposition 5; average-effect bootstrap by Lemma 1 | Five components, each estimate at least 0.05 and lower bound above zero, IUT/Holm | T1 |
| H2 state-to-action influence | $\delta_{\mathrm{off}}=\mu(p,\iota_{\mathrm{off}})-\mu(p,\iota_0)$ | A1–A4, A7–A9; Theorem 2; surgical gain-zero intervention must be evidenced | Paired eligible-prefix difference; exact assignment analysis plus lineage bootstrap | H2 IUT component; powered practical equivalence can refute scoped influence | F2 |
| H2 persistence | $\delta_{\mathrm{reset}}=\mu(p,\iota_{\mathrm{reset}})-\mu(p,\iota_0)$ | A1–A4, A7–A9; Theorem 2; no-rehydration evidence is open | Same paired intervention estimator | Required H2 component plus reset/no-rehydration gate | F2 |
| H2 controlled component sensitivity and dose ordering | $\delta_d^C,\delta_d^D$ in (7g) | A1–A4, A8–A9; Theorem 2; only eligible-prefix controlled effects identified | Paired clamp/dose differences, fixed orientation, lineage uncertainty | All coordinates/doses in H2 IUT | F2 |
| H2 wiring-null validity | $\zeta_j$ and two TOST-side margin quantities | A1–A4, A7–A9; Theorem 2; equivalence margins and stochastic support open | Paired sham/no-op/restore estimator | TOST maximum inside H2; restore bytes hard gate | F2 |
| H3 trace-grounded report advantage | $\gamma_b^M,\gamma_b^C,\gamma_b^0,\gamma_b^R$ for unconstrained, uniform, empirical-prior, receipt-only | A5, A7–A10, A12; Theorem 3 identifies finite paired report performance, not awareness | Same-packet paired macro/coverage/specificity/AURC differences; bootstrap validity conditional on rank regularity | H3 IUT, nine-class/support/no-leak gates, Holm after H1+H2 | T2 |
| H4 known-motif surface transfer | Complete H1 vector on surface holdout | A1–A9 plus leakage-free split; Theorem 1 | Complete H1 estimator on fixed surface roster | $p_{H4,S}$ inside H4 IUT and Holm | T2 |
| H4 known-motif repository transfer | Complete H1 vector on repository holdout | A1–A9 plus repository/ancestry exchangeability; Theorem 1 | Complete H1 estimator on fixed repository roster | $p_{H4,R}$ inside H4 IUT and Holm | T2 |
| H5 discrimination rather than inertia | Frozen utility/non-inferiority $\eta_u$ and fixed-denominator co-gates | A1–A5, A8–A9; Theorem 1 and secondary-functional corollary | Paired hierarchy and component bootstrap | All H5 components evaluated once inside H1 | T1 |
...
| A11 | Frozen joint power DGP is an adequate planning model | Pilot nuisance inputs outside support, unstable covariance/attrition/error model, infeasible $G^\star$ | Confirmation no-go or predeclared scope rule; no power claim |
