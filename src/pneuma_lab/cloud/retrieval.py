...
def _publish(staged: Path, destination: Path) -> None:
    ...
    # Windows rejects fsync on a read-only file descriptor. Opening the
    # already-complete staging file without truncation keeps the same bytes
    # while making the durability barrier portable.
