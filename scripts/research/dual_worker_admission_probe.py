...
def _tool_schema(name: str, arguments: dict[str, object]) -> dict[str, object]:
    ...
    for key, value in arguments.items():
        json_type = "integer" if isinstance(value, int) and not isinstance(value, bool) else "string"
...
def _throughput_observation(engine: Any, vllm: Any, *, rung: str) -> dict[str, object]:
    ...
    for index in range(10):
        ...
        if len(token_ids) != 128:
            raise RuntimeError(f"throughput fixture {index} returned {len(token_ids)} tokens, expected 128")
        samples.append(
            {
                "index": index,
...
def _output_parity_observations(engine: Any, vllm: Any) -> list[dict[str, object]]:
    ...
    for fixture_id in OUTPUT_PARITY_FIXTURE_IDS:
        ...
        if len(first) != 32 or len(second) != 32:
            raise RuntimeError("output-parity fixture did not produce exactly 32 token IDs")
        observations.append(
            {
                "fixture_id": fixture_id,
...
def main() -> int:
    ...
    if tuple(args.rung) != RUNG_NAMES:
        parser.error("supply --rung l40s-tp1-32768 followed by --rung l40s-tp1-65536")
    ...
    if "L40S" not in properties.name:
        raise RuntimeError(f"admission probe requires an L40S, found {properties.name!r}")
    raw = {
        ...
        **bound_hashes,
        ...
        "rungs": {
            rung: _measure_rung(
                model=args.model, revision=args.revision, rung=rung, torch=torch, vllm=vllm
            )
            for rung in args.rung
        },
    }
    _write_new(args.output, canonical_bytes(raw) + b"\n")
    print(json.dumps({"status": "measured", "worker_index": args.worker_index}, sort_keys=True))
    return 0
